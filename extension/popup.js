...
async function refreshState() {
  ...
  els.generate.disabled = !(currentState?.transcriptUrl || currentState?.videoUrl);
}

async function onGenerate() {
  if (!currentState?.transcriptUrl && !currentState?.videoUrl) return;
...
async function runTranscriptPipeline() {
  ...
  const fetchResult = await chrome.runtime.sendMessage({
    ...
    url: currentState.transcriptUrl,
  });
  ...
  else setStatus('Sending to local server (LLaMA is generating notes — ~20-40s)...', 'info');
  ...
  const r = await fetch(`${SERVER_URL}/generate`, {
    ...
    body: JSON.stringify({
      ...
      lessonUrl: currentState.lessonUrl,
...
async function runVideoPipeline() {
  ...
  setStatus(
    'Downloading lecture video → ffmpeg → Whisper → notes.\n' +
...
async function handleServerResponse(r) {
  ...
  setStatus(`✅ Saved: ${data.filename}\n(${data.wordCount.toLocaleString()} words)`, 'success');
